...
void CSkyBox::Draw()
{
	...
	glUniformMatrix4fv(m_pMesh->GetShader()->GetUniforms()[VIEWMAT], 1, GL_FALSE, glm::value_ptr(gSys->pGame->pCamera->GetViewMatrix()));
	glUniformMatrix4fv(m_pMesh->GetShader()->GetUniforms()[PROJMAT], 1, GL_FALSE, glm::value_ptr(gSys->pGame->pCamera->GetProjectionMatrix()));
